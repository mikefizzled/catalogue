...
fetch(`../api/get_single_animal.php?animal_id=${animalId}`)
	...
	.then(data =>
	{
		...
		document.getElementById('thumbnail').src = '../images/thumbnails/' + data.thumbnail;
		document.getElementById('main-picture').src = '../images/thumbnails/' + data.thumbnail;
...
fetch(`../api/get_animal_photos.php?animal_id=${animalId}`)
	...
	.then(data =>
	{
		...
		data.forEach(animal_image =>
		{
			...
			col.innerHTML = `
          <img src="../images/catalogue/${animal_image.filename}" class="d-block img-fluid" alt="${animal_image.caption ? animal_image.caption : 'A picture of a ' + animal_image.common_name}">
          <div>
